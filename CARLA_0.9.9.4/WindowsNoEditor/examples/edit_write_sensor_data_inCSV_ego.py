...
def main():
    ...
    try:
        ...
        with open('rgb_cam_file.csv', mode='w', newline='') as rgb_cam:
            rgb_cam_writer = csv.writer(rgb_cam, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            rgb_cam_writer.writerow(
        ...
        def cam_callback(image):
            ...
            #       )
            with open('rgb_cam_file.csv', mode='a', newline='') as rgb_cam:
        ...
        ego_col = world.spawn_actor(col_bp, col_transform, attach_to=ego_vehicle,
                                    attachment_type=carla.AttachmentType.Rigid)
        with open('colli_file.csv', mode='w', newline='') as colli:
            colli_writer = csv.writer(colli, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            colli_writer.writerow(
                ['frame', 'timestamp', 'x', 'y', 'z', 'pitch', 'yaw', 'roll', 'actor', 'other_actor', 'normal_impulse'])

        def col_callback(colli):
            print("Collision detected:\n" + str(colli) + '\n')
            with open('rgb_cam_file.csv', mode='a', newline='') as col:
        ...
        ego_lane = world.spawn_actor(lane_bp, lane_transform, attach_to=ego_vehicle,
                                     attachment_type=carla.AttachmentType.Rigid)
        with open('lane_file.csv', mode='w', newline='') as lane:
            lane_writer = csv.writer(lane, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            lane_writer.writerow(
                ['frame', 'timestamp', 'x', 'y', 'z', 'pitch', 'yaw', 'roll', 'actor', 'crossed lane markings'])

        def lane_callback(lane):
            print("Lane invasion detected:\n" + str(lane) + '\n')
            with open('lane_file.csv', mode='a', newline='') as lan:
                ...
                lane_app.writerow(
                    [lane.frame, lane.timestamp, lane.transform.location.x, lane.transform.location.y,
                     ...
                     lane.transform.rotation.roll, lane.actor, lane.crossed_lane_markings])
        ...
        ego_obs = world.spawn_actor(obs_bp, obs_transform, attach_to=ego_vehicle,
                                    attachment_type=carla.AttachmentType.Rigid)
        with open('obs_file.csv', mode='w', newline='') as obs:
            obs_writer = csv.writer(obs, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            obs_writer.writerow(
                ['frame', 'timestamp', 'x', 'y', 'z', 'pitch', 'yaw', 'roll', 'actor', 'other actor', 'distance'])

        def obs_callback(obs):
            print("Obstacle detected:\n" + str(obs) + '\n')
            with open('obs_file.csv', mode='a', newline='') as obsta:
                ...
                obs_app.writerow(
                    [obs.frame, obs.timestamp, obs.transform.location.x, obs.transform.location.y,
                     ...
                     obs.transform.rotation.roll, obs.actor, obs.other_actor, obs.distance])
        ...
        ego_gnss = world.spawn_actor(gnss_bp, gnss_transform, attach_to=ego_vehicle,
                                     attachment_type=carla.AttachmentType.Rigid)
        with open('gnss_file.csv', mode='w', newline='') as gnss:
            gnss_writer = csv.writer(gnss, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            gnss_writer.writerow(
                ['frame', 'timestamp', 'x', 'y', 'z', 'pitch', 'yaw', 'roll', 'altitude', 'latitude', 'longitude'])

        def gnss_callback(gnss):
            print("GNSS measure:\n" + str(gnss) + '\n')
            with open('gnss_file.csv', mode='a', newline='') as gns:
        ...
        ego_imu = world.spawn_actor(imu_bp, imu_transform, attach_to=ego_vehicle,
                                    attachment_type=carla.AttachmentType.Rigid)
        with open('imu_file.csv', mode='w', newline='') as imu:
            imu_writer = csv.writer(imu, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            imu_writer.writerow(
                ['frame', 'timestamp', 'x', 'y', 'z', 'pitch', 'yaw', 'roll', 'accelerometer x', 'accelerometer y',
                 'accelerometer z', 'compass', 'gyroscope x', 'gyroscope y', 'gyroscope z'])

        def imu_callback(imu):
            print("IMU measure:\n" + str(imu) + '\n')
            with open('imu_file.csv', mode='a', newline='') as imuu:
        ...
        def Log_Depth_callback(image):
            ...
            with open('log_depth_cam_file.csv', mode='a', newline='') as log_depth_ca:
        ...
        sem_cam = world.spawn_actor(sem_bp, sem_transform, attach_to=ego_vehicle,
                                    attachment_type=carla.AttachmentType.Rigid)
        with open('sem_cam_file.csv', mode='w', newline='') as sem_camm:
            sem_cam_writer = csv.writer(sem_camm, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        ...
        def sem_callback(image):
            ...
            with open('sem_cam_file.csv', mode='a', newline='') as sem:
        ...
        with open('lidar_data_file.csv', mode='w', newline='') as lidar_data:
            ...
            lidar_data_writer.writerow(
                ['frame', 'timestamp', 'x', 'y', 'z', 'pitch', 'yaw', 'roll', 'channels', 'horizontal_angle',
                 'raw data'])
