...
class World(object):
    ...
    def restart(self):
        ...
        self.obs_sensor = ObstacleDetector(self.player)
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        with open('edit_sensor_data_sort.csv', mode='a', newline='') as col:
            ...
            colli_app.writerow(
                ['v_file:', 'frame', 'timestamp', 'x', 'y', 'z', 'pitch', 'yaw', 'roll', 'actor', 'other_actor',
                 'normal_impulse'])
            colli_app.writerow(
                ['', event.frame, event.timestamp, event.transform.location.x, event.transform.location.y,
                 event.transform.location.z, event.transform.rotation.pitch, event.transform.rotation.yaw,
                 event.transform.rotation.roll, event.actor, event.other_actor, event.normal_impulse])
...
class LaneInvasionSensor(object):
    ...
    def _on_invasion(weak_self, event):
        ...
        #         ['', event.frame, event.timestamp, event.transform.location.x, event.transform.location.y,
        #          event.transform.location.z, event.transform.rotation.pitch, event.transform.rotation.yaw,
        #          event.transform.rotation.roll, event.actor.id, event.crossed_lane_markings])
...
class GnssSensor(object):
    ...
    def _on_gnss_event(weak_self, event):
        ...
        #         ['', event.frame, event.timestamp, event.transform.location.x, event.transform.location.y,
        #          event.transform.location.z, event.transform.rotation.pitch, event.transform.rotation.yaw,
        #          event.transform.rotation.roll, event.altitude, event.latitude, event.longitude])
...
class IMUSensor(object):
    ...
    def _IMU_callback(weak_self, sensor_data):
        ...
        #     imu_app.writerow(
        #         ['', sensor_data.frame, sensor_data.timestamp, sensor_data.transform.location.x, sensor_data.transform.location.y,
        #          sensor_data.transform.location.z, sensor_data.transform.rotation.pitch, sensor_data.transform.rotation.yaw,
        #          sensor_data.transform.rotation.roll, sensor_data.accelerometer.x, sensor_data.accelerometer.y, sensor_data.accelerometer.z,
        #          sensor_data.compass, sensor_data.gyroscope.x, sensor_data.gyroscope.y, sensor_data.gyroscope.z])
...
class ObstacleDetector(object):
    def __init__(self, parent_actor):
        ...
        bp.set_attribute('distance', str(50))
        # bp.set_attribute('hit_radius', str(6))
        # bp.set_attribute('sensor_tick', str(2))
        self.sensor = world.spawn_actor(
            bp, carla.Transform(), attach_to=self._parent)
        # We need to pass the lambda a weak reference to self to avoid circular
        # reference.
        weak_self = weakref.ref(self)
        self.sensor.listen(
            lambda obs: ObstacleDetector._Obs_callback(weak_self, obs))

    @staticmethod
    def _Obs_callback(weak_self, obs):
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        # with open('edit_sensor_data_sort.csv', mode='a', newline='') as cam:
        #     cam_app = csv.writer(cam, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        #     cam_app.writerow(
        #         ['cam_file:', 'frame', 'timestamp', 'x', 'y', 'z', 'pitch', 'yaw', 'roll', 'field of view',
        #          'height', 'width',
        #          'raw data'])
        #     cam_app.writerow(
        ...
        # if image.frame % 11 == 0:
        #     image.save_to_disk('../images/images-%d.png' % (image.frame))
        #     with open('../images/images.txt', mode='a', newline='\n') as im:
        #         im.write('/mydrive/images/images-%d.png\n' % (image.frame))
