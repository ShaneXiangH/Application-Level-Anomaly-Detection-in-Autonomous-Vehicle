    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    while (1) {
        ...
        draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output, fpWrite);
    ...
    int letter_box, int benchmark_layers)
{
    ...
    int j;
    float nms = .45;    // 0.4F
    FILE *fpWrite = fopen("dataa.txt", "w");
    while (1) {
        if (filename) {
            strncpy(input, filename, 256);
            if (strlen(input) > 0)
        ...
        draw_detections_v3(sized, dets, nboxes, thresh, names, alphabet, l.classes, 1, fpWrite);
